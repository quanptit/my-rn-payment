...
//        reactContext.addActivityEventListener(this);
    ...
    @ReactMethod
    public void purchaseProduct(String productId, String IAP_LICENSE_KEY, final Promise promise) {
        this.promise = promise;
        Activity activity = getCurrentActivity();
        new IAPUtils(activity, new IAPUtils.IABCallback() {
            @Override public void error(String errorMessage) {
                if (RNIAPModule.this.promise != null)
                    RNIAPModule.this.promise.reject("0", errorMessage);
            }

            @Override public void success() {
                if (RNIAPModule.this.promise != null)
                    RNIAPModule.this.promise.resolve(true);
            }
        }).purchase(productId);
//        if (activity != null) {
