...
export class DialogPaymentChoose extends Component {
    ...
    render() {
        ...
        if (this.isEnableTheCao) {
            buttons.push(<Button key={buttons.length} title="Nâng cấp qua thẻ cào điện thoại" model={ButtonModel.primary} style={{ margin: 12 }} onPress={() => {
                if (isIOS())
                    DialogUtils.showDialog(<DialogTheoCao style={{ justifyContent: "flex-start", paddingTop: 20 }} callbackUpdateVip={this.callbackUpdateVip.bind(this)}/>);
            }}/>);
        }
        buttons.push(<Button key={buttons.length} title={getStringsCommon().Nang_cap_ngay} model={ButtonModel.warning} style={{ marginLeft: 12, marginRight: 12, marginTop: 12, marginBottom: isIOS() ? 0 : 12 }} onPress={this.btnIAPClick.bind(this)}/>);
        if (isIOS())
            buttons.push(<Button key={buttons.length} title={"Did you buy earlier? Restore now"} model={ButtonModel.light} style={{ margin: 12 }} onPress={this.btnRestoreClick.bind(this)}/>);
        return (<PopupDialog {...this.props} width={"90%"} ref={(popupDialog) => { this.popupDialog = popupDialog; }}>
